...
class DataCapture(object): #object per simulation
	...
	def to_csv_path(self):
		df_list = []
		for i, value in enumerate(self.player_list):
			for j, route_value in enumerate(value.positions):
				print(route_value)
				df_list.append([value.veh_id, j, route_value[0], route_value[1]])

		pd.DataFrame(df_list,columns=['id', "timestamp", "x", "y"]).to_csv("pathoutput.csv")
...
def plot_others_graph(folder, plot_values = "reward", y_axis="ru", box_plot=None, avg_player=False, to_csv=False, horizontal=False, scatter=False, tTest=False, normalize=False, error_bar=False, save=False, combine_graph=False, ylim=None, xlim=None):
	...
	for group in divided_list:
		...
		for i, single_file in enumerate(group):
			...
			elif plot_values == "bufferinterval" or "bufferpoi":
   ...
   # df = pd.DataFrame(df_list, columns=[plot_values,'algo','rc','ru',"player_amount", "cells", "ru_over_base", "reward_visited", "average_utility", "capacity_mean", "average_utilization", "average_coverage", "reward_visited_total"])
	...
	if (not box_plot):
		...
		if not combine_graph:
			...
			g.add_legend(title="", loc="upper left", ncol = 4,columnspacing=0, handletextpad=0, labelspacing=0, prop={'size':6.5}, borderpad=0, fancybox=True, framealpha=0)#, labels=label_order)
...
def plot_time_series(folder, plot_values = "sim", y_axis="rc", heatmap=False,to_csv=False):
...
class DataCaptureGraph(DataCapture): #object per simulation
	...
	def compute_spatial_coverage(self, custom_player_list=None, sample_rate = 50, heatmap=False):
		total_path = []
		path_no_sample = []
		...
		for j, player in enumerate(self.player_list):
			#route_sample.append(player.routes)
			#node_hit.append(player.node_hit)
			...
			total_path.append(player_array_np[::sample_rate])
			path_no_sample.append(player_array_np)
		...
		#del total_path[8]
		#del total_path[11]
		fdist_dict = find_fdist(total_path[:5], player_list=temp_player_list)
		#fdist_dict = find_fdist(total_path, player_list=temp_player_list)
